...
let copy = {
  ...
  GEO: {
    ...
    heroText: 'პრემიუმ გაყიდვების განათლება, მოლაპარაკების სტრატეგია და პერსონალური ბრენდის განვითარება ლიდერებისთვის, რომლებსაც სურთ უფრო ზუსტი გავლენა, სწრაფი გადაწყვეტილებები და გაზომვადი შედეგები.',
...
const WHATSAPP_CHAT_URL = 'https://wa.me/995577208606';
...
async function handleContactSubmit(event, lang, setFormStatus, setIsSubmitting) {
  ...
  try {
    ...
    if (!response.ok || result.ok === false) {
      ...
      if (/non-browser environments|Enable this option|API access/i.test(message)) {
...
function WhatsAppFloatingButton({ lang }) {
  const t = copy[lang];
  const [isOpen, setIsOpen] = React.useState(false);
  const [whatsAppForm, setWhatsAppForm] = React.useState({ name: '', phone: '', message: '' });
  const sendLabel = lang === 'GEO' ? 'გაგზავნა' : 'Send';
  ...
  const handleWhatsAppSubmit = () => {
    ...
    const phoneValue = whatsAppForm.phone.trim();
    ...
    const phone = (buttonLinks.whatsapp || WHATSAPP_CHAT_URL).replace('https://wa.me/', '');
    ...
    isOpen &&
      h(
        ...
        h(
          ...
          h('p', { className: 'text-xs font-black uppercase tracking-[0.26em] text-luxuryRed' }, lang === 'GEO' ? 'WhatsApp ჩატი' : 'WhatsApp Chat'),
          h(
            'button',
            {
              type: 'button',
        ...
        h(
          ...
          h('input', { required: true, value: whatsAppForm.name, onChange: (event) => updateField('name', event.target.value), className: 'whatsapp-chat-input', placeholder: t.name }),
          h('input', { required: true, value: whatsAppForm.phone, onChange: (event) => updateField('phone', event.target.value), className: 'whatsapp-chat-input', placeholder: t.phone }),
          h('textarea', { required: true, value: whatsAppForm.message, onChange: (event) => updateField('message', event.target.value), className: 'whatsapp-chat-input min-h-28 resize-none py-3', placeholder: t.message }),
          h('button', { type: 'button', onClick: handleWhatsAppSubmit, className: 'site-cta whatsapp-chat-submit' }, sendLabel)
        )
      ),
    h(
      'button',
      ...
      h('span', null, lang === 'GEO' ? 'WhatsApp ჩატი' : 'WhatsApp Chat')
    )
  );
}
...
function App() {
  ...
  return h(React.Fragment, null, h('div', { className: 'luxury-light-field', 'aria-hidden': 'true' }), h(Header, { lang, setLang }), h('main', null, h(Hero, { lang }), h(About, { lang }), h(Courses, { lang }), h(BookGallery, { lang }), h(Programs, { lang }), h(WinSpaceSection, { lang }), h(Testimonials, { lang }), h(Contact, { lang })), h(Footer, { lang }), h(WhatsAppFloatingButton, { lang }));
}
